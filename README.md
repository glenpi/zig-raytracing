...
in `main.zig` is the book's "final render" scene — a field of randomly
...
Skipping `-O ReleaseFast` also works, but is dramatically slower — Zig's
default Debug build adds bounds/overflow checking that matters a lot for
a pixel-by-pixel, ray-per-sample workload like this one. Worth knowing
